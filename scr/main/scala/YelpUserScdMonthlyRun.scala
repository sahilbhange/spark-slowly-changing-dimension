...
object YelpUserScdMonthlyRun {
  def main(args: Array[String]): Unit = {
    ...
    val yelp_data_upnew = yelp_data_hist.as("hist").filter($"exp_dt"==="2099-12-31").
                                        ...
                                        where($"stg.user_id".isNull || $"hist.review_count" =!= $"stg.review_count" || 
                                                ...
                                                $"hist.compliment_funny" =!= $"stg.compliment_funny" || 
                                                $"hist.compliment_writer" =!= $"stg.compliment_writer" || 
                                                $"hist.compliment_photos" =!= $"stg.compliment_photos").select($"hist.user_id",$"stg.*")
    ...
    // Get old updated user_id from the HIST table and expiry date i.e. (2099-12-31)
